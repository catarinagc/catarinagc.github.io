...
const container2 = document.getElementById('mainZone');
...
const material2 = new THREE.MeshBasicMaterial({ color: "rgb(235, 234, 176)" });
const material3 = new THREE.MeshBasicMaterial({ color: "rgb(216, 149, 177)" });
const materialArray = [material1, material2, material3];

// --- Scene dimensions ---
const fov = camera.fov * (Math.PI / 180);
const depth = 250;
let visibleHeight = 2 * Math.tan(fov / 2) * depth;
...
for (let i = 0; i < 1500; i++) {
  ...
  const xPos = THREE.MathUtils.randFloat(-visibleWidth / 2, visibleWidth / 2);
  const yPos = THREE.MathUtils.randFloat(-visibleHeight / 2, visibleHeight / 2);
  circle.position.set(xPos, yPos, -depth);
  scene.add(circle);
  circles.push({ mesh: circle, originalX: xPos, originalY: yPos });
}
...
// --- Animation loop ---
function animate() {
  const worldMouse = getMousePositionInWorld();
  const mouseX = worldMouse.x;
  const mouseY = worldMouse.y;
  const range = 25;

  circles.forEach(({ mesh, originalX, originalY }) => {
    const offsetX = (mouseX - originalX) * 0.1;
    const offsetY = (mouseY - originalY) * 0.1;
